...
import { api, DataPaging, User, UserQuery } from "../lib/tauri";
...
// Column definitions
// ─────────────────────────────────────────────
type ColKey = "full_name" | "username" | "role" | "is_active" | "sync" | "created_at";
type SortableKey = Extract<UserQuery["sort_by"], string>;
...
const DEFAULT_COLS: ColDef[] = [
  { key: "full_name", label: "Nama", visible: true, sortBy: "full_name", width: "w-24" },
  { key: "username", label: "Username", visible: true, sortBy: "username", width: "w-24" },
  { key: "role", label: "Role", visible: true, sortBy: "role", width: "w-20" },
...
type UsersPageProps = {
  mode?: "page" | "modal";
  onSelect?: (value: User) => void; // optional
};

export default function UsersPage({ mode = "page", onSelect }: UsersPageProps) {
  ...
  useEffect(() => { setPage(1); }, [debouncedSearch, filterSync, sortBy, sortDir, perPage, filterRole]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const query: UserQuery = {
        ...
        ...(filterSync as UserQuery["sync_status"] && { sync_status: filterSync as UserQuery["sync_status"] }),
        ...(filterRole as UserQuery["role"] && { role: filterRole as UserQuery["role"] }),
      };
  ...
  }, [debouncedSearch, filterSync, sortBy, sortDir, perPage, filterRole, page]);
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title="Pengguna" subtitle="Maks. 1 user aktif per role"
        actions={mode == "page" ? (<Button onClick={() => setAddOpen(true)}>+ Tambah User</Button>) : undefined} />
                  ...
                  <tr key={p.id} className={clsx(
                    ...
                    {visibleCols.map((col) => (
                      <td key={col.key}
                        ...
                        {col.key === "username" && (
                          <span className="font-mono text-xs text-gray-500">{p.username}</span>
                        )}
                        {col.key === "role" && (
